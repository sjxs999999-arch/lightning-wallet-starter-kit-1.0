...
export const configSchema = z.object({
  ...
  EVM_RPC_FALLBACK_URLS:z.string().optional(),SOLANA_RPC_URL:z.string().url().default('https://solana-rpc.publicnode.com'),SOLANA_RPC_FALLBACK_URLS:z.string().default('https://solana.drpc.org,https://api.mainnet-beta.solana.com'),TRON_RPC_URL:z.string().url().default('https://api.trongrid.io'),TRON_RPC_FALLBACK_URLS:z.string().optional(),TRONGRID_API_KEY:z.preprocess(value=>value===''?undefined:value,z.string().min(8).max(256).optional()),RATE_LIMIT_MAX:z.coerce.number().int().positive().default(120),METRICS_TOKEN:z.preprocess(value=>value===''?undefined:value,z.string().min(32).max(256).optional())

...
const input = { chain: 'EVM', chainId: 1, sellToken: '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', buyToken: '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', sellAmount: '1000000', sellDecimals: 6, taker: '0x42BAe181b2Fbd5cc8F04762770942C719Dd4d30a', slippageBps: 50 };
const quote = { id: 'quote-1', tool: 'sushiswap', toolDetails: { name: 'SushiSwap Aggregator' }, action: { fromChainId: 1, toChainId: 1, fromAmount: input.sellAmount, fromAddress: input.taker, toAddress: input.taker, fromToken: { address: input.sellToken, symbol: 'USDC', decimals: 6 }, toToken: { address: input.buyToken, symbol: 'WETH', decimals: 18 } }, estimate: { fromAmount: input.sellAmount, toAmount: '436232556501834', toAmountMin: '434051393719324', approvalAddress: '0x1231DEB6f5749EF6cE6943a275A1D3E7486F4EaE', fromAmountUSD: '0.9993', toAmountUSD: '0.9960', feeCosts: [{ amountUSD: '0.0025' }], gasCosts: [{ amountUSD: '0.7592' }] }, transactionRequest: { chainId: 1, from: input.taker, to: '0x1231DEB6f5749EF6cE6943a275A1D3E7486F4EaE', data: '0x1234', value: '0', gasLimit: '651253' } };
...
describe('Vercel public EVM Swap parity', () => {
  it('keeps status and metadata-only quotes public while jobs stay operator protected', () => {
    ...
    expect(routeRequiresAuth('GET', 'swap/token-metadata')).toBe(false);
  ...
  it('strictly normalizes matching same-chain transaction data', () => {
    expect(normalizeLiFiEvmQuote(input, quote, 1_700_000_000_000)).toMatchObject({ provider: 'LI.FI / SushiSwap Aggregator', amountIn: input.sellAmount, feeUsd: '0.0025', gasCostUsd: '0.7592', display: { amountIn: '1', sellSymbol: 'USDC', sellDecimals: 6, buyDecimals: 18 }, transaction: { to: quote.transactionRequest.to, data: '0x1234', value: '0x0' }, raw: { source: 'LI.FI', sameChain: true } });
  ...
  it('rate-limits the anonymous route without requiring an operator session', () => {
    ...
    expect(route).toContain("anonymousRateLimit(req,'swap-quotes',30)");
    expect(route).toContain('SWAP_QUOTE_RATE_LIMIT');
    expect(route).not.toContain('requireAuth');
    expect(source).toContain("anonymousRateLimit(req,'swap-metadata',30)");
    expect(source).toContain("interval '1 minute'");
    expect(source).toContain('swapMetadataCache');
  });
});

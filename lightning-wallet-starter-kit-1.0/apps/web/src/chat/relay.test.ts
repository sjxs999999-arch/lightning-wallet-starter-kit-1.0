...
describe('chat relay client', () => {
  it('uses a server challenge bound to the domain, wallet and device', async () => {
    ...
    const message = ['Lightning Wallet Chat Authentication', 'Domain: wallet.br.com', 'Purpose: chat-device-bind', 'Chain: EVM', `Address: ${address}`, `Device ID: ${device.id}`, `Device key: ${device.publicKey}`, 'This request does not authorize a transaction or token approval.'].join('\n');
  ...
  it('rejects a challenge that can be replayed against another device', async () => {
    const device = createChatDevice(), address = `0x${'a'.repeat(40)}`;
    const fetcher = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => json({ data: { challengeId: crypto.randomUUID(), challenge: {}, message: `wallet.br.com chat-device-bind EVM ${address} another-device ${device.publicKey}`, expiresAt: new Date(Date.now() + 300_000).toISOString() } }));

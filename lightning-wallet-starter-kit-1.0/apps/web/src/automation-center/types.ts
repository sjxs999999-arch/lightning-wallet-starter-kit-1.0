export type RuleKind='price'|'watchlist'|'portfolio'|'gas'|'health';export type JobKind=RuleKind|'notification';export type ChannelKind='telegram'|'email'|'webhook';
...
export type AutomationJob={id:string;rule_id:string|null;rule_name:string;kind:JobKind;status:'queued'|'completed'|'failed';attempt:number;dry_run:boolean;detail:Record<string,unknown>;error?:string;created_at:string;finished_at?:string};

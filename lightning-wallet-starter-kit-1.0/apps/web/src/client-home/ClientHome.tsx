...
type Readiness = { finalApproval: boolean; walletAcceptanceRequired: boolean; externalBlockers: { code: string; label: string }[]; mainnet: { readOnlyWalletConnection: boolean; execution: boolean; swap: boolean; launchpad: boolean; bridge: boolean } };
type CapabilityResponse = { data: { version: string; chains: string[]; features: Capability[]; readiness?: Readiness; security: { privateKeysUploaded: boolean; serverSigning: boolean; serverBroadcast: boolean } } };
...
export function ClientHome() {
  ...
  return <>
    ...
    <div className="client-home-stats">
      <section className="panel"><span>客户端版本</span><strong>{capabilities?.version ?? '2.39.0'}</strong><small>正式运行 · 交易门禁</small></section>
    ...
    {capabilities?.readiness && <section className={`client-readiness ${capabilities.readiness.finalApproval ? 'approved' : 'blocked'}`}><ShieldCheck size={18}/><div><b>{capabilities.readiness.finalApproval ? '最终生产验收已通过' : '主网只读已上线，交易执行继续受门禁保护'}</b><span>主网只读连接：{capabilities.readiness.mainnet.readOnlyWalletConnection ? '已开启' : '关闭'} · {capabilities.readiness.externalBlockers.length} 项外部依赖未完成{capabilities.readiness.walletAcceptanceRequired ? ' · 钱包签名验收待完成' : ''} · 主网执行：{capabilities.readiness.mainnet.execution ? '已开启' : '关闭'}</span></div></section>}

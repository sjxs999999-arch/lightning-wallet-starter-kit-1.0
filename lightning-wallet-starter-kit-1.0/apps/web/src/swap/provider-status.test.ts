...
describe('swap provider status fallback', () => {
  it('keeps public Jupiter and SUN.io quote paths available when status cannot be read', () => {
    ...
    expect(result.find(item => item.chain === 'EVM')).toMatchObject({ available: false });
  });
});

...
describe('client-only transaction executor',()=>{
  ...
  it('rejects a lookalike TRON RPC before calling the token contract',async()=>{const at=vi.fn(),tronWeb={defaultAddress:{base58:'TPxqxJiNbT5XNbQFuC1LNX2pyEztrJcJEA'},fullNode:{host:'https://nile.trongrid.io.attacker.test'},trx:{sendTransaction:vi.fn()},contract:()=>({at})};window.okxwallet={request:vi.fn(),tronLink:{request:vi.fn(async()=>({code:200})),tronWeb}};await expect(executeTask({...task,chain:'TRON',assetKind:'token',from:'TPxqxJiNbT5XNbQFuC1LNX2pyEztrJcJEA',to:'TQxgyuuj43UrFhYtgkBGNTZtLY4iuvm7CL',amount:'1',token:'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t',decimals:6})).rejects.toThrow(/官方网络/);expect(at).not.toHaveBeenCalled()});

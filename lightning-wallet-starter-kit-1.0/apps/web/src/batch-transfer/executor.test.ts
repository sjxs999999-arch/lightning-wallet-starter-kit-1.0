...
describe('client-only transaction executor',()=>{
  ...
  it('blocks unapproved EVM mainnet address pairs',async()=>{window.ethereum={request:vi.fn(async({method})=>method==='eth_requestAccounts'?[task.from]:'0x38')};await expect(executeTask({...task,amount:'0.0001'})).rejects.toThrow('地址对');expect(window.ethereum.request).toHaveBeenCalledTimes(2)});

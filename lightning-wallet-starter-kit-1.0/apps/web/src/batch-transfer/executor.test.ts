...
describe('client-only transaction executor',()=>{
  ...
  it('blocks EVM mainnet broadcasting',async()=>{window.ethereum={request:vi.fn(async()=> '0x1')};await expect(executeTask(task)).rejects.toThrow('Sepolia');expect(window.ethereum.request).toHaveBeenCalledTimes(1)});

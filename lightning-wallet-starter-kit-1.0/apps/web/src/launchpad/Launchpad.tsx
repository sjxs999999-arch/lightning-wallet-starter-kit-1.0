...
const networks: Record<LaunchChain, LaunchDraft['network']> = { EVM: 'sepolia', SOL: 'solana-devnet', TRON: 'tron-nile' };
...
export function Launchpad() {
  ...
  function chain(value: LaunchChain) {
    setDraft(current => ({ ...current, chain: value, network: networks[value], decimals: defaults[value] })); resetDeployment();
  }
  ...
  return <>
    <div className="page-head"><div><p className="eyebrow">MULTICHAIN TOKEN LAUNCH</p><h1>闪电发射台</h1><p>创建 Token 资料、规划初始流动性，并由用户钱包完成测试网部署。</p></div></div>

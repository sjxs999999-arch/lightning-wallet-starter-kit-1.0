...
const tokenDecimals = value => Number.isInteger(value) && value >= 0 && value <= 30 ? value : null;
const tokenSymbol = (value, fallback) => typeof value === 'string' && /^[A-Za-z0-9._-]{1,24}$/.test(value) ? value : fallback;
const formatAtomic = (raw, decimals) => { if (decimals === 0) return raw; const value = raw.padStart(decimals + 1, '0'); return `${value.slice(0, -decimals)}.${value.slice(-decimals)}`.replace(/\.0+$/, '').replace(/(\.\d*?)0+$/, '$1'); };
...
export function normalizeLiFiEvmQuote(input, payload, now = Date.now()) {
  ...
  const raw = record(payload), action = record(raw.action), estimate = record(raw.estimate), transaction = record(raw.transactionRequest), fromToken = record(action.fromToken), toToken = record(action.toToken), sellDecimals = tokenDecimals(fromToken.decimals), buyDecimals = tokenDecimals(toToken.decimals);
  ...
  if (Number(action.fromChainId) !== input.chainId || Number(action.toChainId) !== input.chainId || Number(transaction.chainId ?? action.fromChainId) !== input.chainId || !sameAddress(action.fromAddress, input.taker) || !sameAddress(action.toAddress, input.taker) || transaction.from !== undefined && !sameAddress(transaction.from, input.taker) || action.fromAmount !== input.sellAmount || amountIn !== input.sellAmount || !tokenMatches(fromToken, input.sellToken) || !tokenMatches(toToken, input.buyToken) || sellDecimals === null || buyDecimals === null || sellDecimals !== input.sellDecimals || !amountOut || !minReceived || BigInt(minReceived) > BigInt(amountOut) || !to || !data || !value) throw new Error('LIFI_EVM_ROUTE_MISMATCH');
  ...
  return { provider: `LI.FI / ${toolName}`, amountIn, amountOut, minReceived, priceImpactPct: impact, route: labels([tool, ...(Array.isArray(raw.includedSteps) ? raw.includedSteps.map(step => record(step).tool) : [])]), allowanceTarget, transaction: { to, data, value, ...(gas ? { gas } : {}), ...(gasPrice ? { gasPrice } : {}) }, feeUsd: usdString(usdTotal(estimate.feeCosts)), gasCostUsd: usdString(usdTotal(estimate.gasCosts)), expiresAt: new Date(now + 55_000).toISOString(), display: { amountIn: formatAtomic(amountIn, sellDecimals), amountOut: formatAtomic(amountOut, buyDecimals), minReceived: formatAtomic(minReceived, buyDecimals), sellSymbol: tokenSymbol(fromToken.symbol, 'ERC-20'), buySymbol: tokenSymbol(toToken.symbol, 'ERC-20'), sellDecimals, buyDecimals, usdValuationAvailable: toUsd > 0 }, raw: { source: 'LI.FI', quoteId: typeof raw.id === 'string' ? raw.id.slice(0, 120) : undefined, tool, chainId: input.chainId, sameChain: true } };
}

...
if [ -n "$wallet_acceptance_report" ] || [ -n "$wallet_acceptance_sha" ]; then
  ...
  elif ! EXPECTED_RELEASE=2.41.0 FINAL_WALLET_ACCEPTANCE_EVIDENCE_SHA256="$wallet_acceptance_sha" "$SCRIPT_DIR/verify-wallet-acceptance.sh" "$wallet_acceptance_report"; then
...
if [ "$wallet_acceptance" = true ] && [ -n "$wallet_acceptance_report" ] && [ -n "$wallet_acceptance_sha" ]; then
  ...
  if ! EXPECTED_RELEASE=2.41.0 FINAL_WALLET_ACCEPTANCE_EVIDENCE_SHA256="$wallet_acceptance_sha" \

...
ROLLBACK_EXECUTE=${ROLLBACK_EXECUTE:-false}
ROLLBACK_CONFIRM=${ROLLBACK_CONFIRM:-}
ROLLBACK_SKIP_PREFLIGHT=${ROLLBACK_SKIP_PREFLIGHT:-false}
COMPOSE_PROJECT_NAME=${COMPOSE_PROJECT_NAME:-lightning-wallet}

usage() {
  echo "Usage: $0 <release-id>" >&2
  echo 'Default mode is a non-mutating plan. Execute only with ROLLBACK_EXECUTE=true and ROLLBACK_CONFIRM=<release-id>.' >&2
  exit 2
}
...
if [ "$ROLLBACK_SKIP_PREFLIGHT" = true ]; then
  [ "$ROLLBACK_EXECUTE" != true ] || { echo 'ROLLBACK_SKIP_PREFLIGHT is forbidden during execution.' >&2; exit 1; }
else
...
[ "$ROLLBACK_CONFIRM" = "$release_id" ] || { echo 'ROLLBACK_CONFIRM must exactly match the target release ID.' >&2; exit 1; }
[ "$(id -u)" -eq 0 ] || { echo 'Executed rollback must run as root.' >&2; exit 1; }

if command -v flock >/dev/null 2>&1; then
  exec 9>/var/lock/lightning-wallet-rollback.lock
  flock -n 9 || { echo 'Another Lightning Wallet rollback is already running.' >&2; exit 1; }
fi
...
if (cd "$target" && PRODUCTION_ENV_FILE=.env.production ./scripts/deploy.sh && ./scripts/verify-production.sh); then
  finished=$(date +%s)
  printf 'Rollback completed in %s seconds. Database data was preserved.\n' "$((finished - started))"
  exit 0
fi

echo 'Target release failed deployment or verification; restoring the original release.' >&2

# Lightning Wallet v2.1 Production Deployment

## Prerequisites

- Linux host with Docker Engine and Docker Compose v2
- A DNS record pointing the production domain to the host
- TLS certificate files at `certbot/conf/live/<domain>/fullchain.pem` and `privkey.pem`
...
2. Replace every placeholder and verify `DOMAIN`, `CORS_ORIGIN`, RPC endpoints and external integration URLs.
...
Verify `https://<domain>/health`, `/health/ready` and `/metrics`. The readiness endpoint must report PostgreSQL and Redis as `ok` before traffic is accepted.
